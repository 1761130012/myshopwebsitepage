  ...
  <div>
    <el-card class="order-item-box" v-for="(item,index) in list" :key="index">
      <div slot="header" class="clearfix">
        ...
        <el-button style="margin-left: 760px; padding: 3px 0" type="text" v-if="item.state===2"
        @click="handleEdit(item.orderId)">去提货
        </el-button>
      </div>
...
export default {
  ...
  methods: {
    getlist() {
      ...
      _this.$axios({url: "order/queryAllOrderByUserIdti", method: "post", data: params}).then(response => {
