  ...
  <div>
    ...
    <el-table :data="orderData" v-loading="loading" width="100%" header-align="center">
      ...
      <el-table-column
        label="操作">
        <template slot-scope="params">
          <el-button type="primary" size="small" @click="queryGoodsByOrderId3(params.row.orderId)">详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="提示" :visible.sync="OpenoutOrderCommit" width="60%">
...
export default {
  ...
  methods: {
    getlist() {
      ...
      _this.$axios.post("order/queryAllOrderByUserIdyi", params).then((response) => {
    ...
    queryGoodsByOrderId3(orderId) {
      let _this=this;
      _this.OpenoutOrderCommit=true;
      let params = new URLSearchParams();
      params.append("current", _this.currentPage3);
      params.append("size", _this.size3);
      ...
      _this.$axios({url:"order/queryOrderShopByOrderId",method:"post",data:params}).then(response=>{
        ...
        _this.total3=response.data.total;
