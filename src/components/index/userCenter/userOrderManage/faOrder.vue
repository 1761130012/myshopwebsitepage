...
<div>
  ...
  <el-table :data="orderData" v-loading="loading" width="100%" header-align="center">
    ...
    <el-table-column
      label="操作">
      <template slot-scope="params">
        <el-button type="primary" size="small" @click="queryGoodsByOrderId1(params.row.orderId)">详情</el-button>
      </template>
    </el-table-column>
  </el-table>
  ...
  <el-dialog title="提示" :visible.sync="OpenfaOrder" width="60%">
    <el-table :data="fahuoData" width="100%">
...
export default {
  ...
  methods: {
    getlist() {
      ...
      _this.$axios.post("order/queryAllOrderByUserIdfa", params).then((response) => {
    ...
    queryGoodsByOrderId1(orderId) {
      let _this=this;
      _this.OpenfaOrder=true;
      let params = new URLSearchParams();
      ...
      _this.$axios({url:"order/queryOrderShopByOrderId",method:"post",data:params}).then(response=>{
        _this.fahuoData=response.data.records;
